...
The first thing I did was to set up the workflows for both platforms: `iOS and `visionOS`.

## The iOS Workflow

I started with the iOS workflow and set up all of its steps and information, which I will go through in this section.

In the general information section, I specified the repository and project to use, as well as checked the 'Restrict Editing' box as it is required for workflows that upload builds for external testing:
...
Both the iOS and visionOS workflows are very similar, so I will only highlight the main differences you need to be aware of.

First, instead of specifying the `iOS/` prefix for the tag, I specified `visionOS/`:

![]()

And in the `Archive` action, I changed the scheme and platform:
...
I also installed a plugin to help me check the `git status` for specific files to then decide on whether to commit the changes or not:
