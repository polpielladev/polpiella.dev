...
Let's start by importing the [ANSITerminal](https://github.com/pakLebah/ANSITerminal) library in the executable's entry point, creating a `picker` function which contains the component's code and invoke it immediately after with a title and a set of options:
...
The logic to render all options in the terminal can be broken down into the following steps:
...
When called, the `picker` method listens for user input by doing the following:
...
The `reRender` method can be called when the picker's state changes to:
