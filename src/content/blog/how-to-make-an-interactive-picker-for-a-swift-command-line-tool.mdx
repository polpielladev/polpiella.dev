...
The first step to creating an interactive command-line component is to create an executable Swift package:
...
The picker method now executes the following steps to set up the initial steps:
...
The code displays all options to the user by executing the following steps:
...
When called, the `picker` method can now listen for user input through the following steps:
...
The `reRender` method can be used to react to user input and:
...
That's it! 🎉 We have written an interactive picker for the command-line in Swift. I am planning on writing more posts on this topic as a build up my Swift port of the [clack](https://github.com/natemoo-re/clack) library, so stay tuned 👀.
