...
In this post, I will show you how to make a simple interactive picker for a command-line tool using ANSI escape sequences inspired in one of my favourite javascript libraries [clack](https://github.com/natemoo-re/clack) by [Nate Moore](https://twitter.com/n_moore). I like the library so much that I have started working on migrating it to Swift and I am going to document my progress in a series of posts.

<StaticTweet id="1631686077399420928" />

## What are ANSI escape sequences?

ANSI escape sequences are a way to add special formatting to text in the terminal through a series of predefined characters. They are supported by terminals in Unix operating systems (like macOS) and Windows (only from Windows 10) and are used by many command-line tools to change the text colour, modify the cursor position, clear the screen, etc.
...
The escape sequence starts with the character `\u{1B}` which is the escape character. This is followed by the character `[` and a number that represents the colour we want to use. In this case, `31` is the code for red. Finally, we end the sequence with the character `m` and then add the text we want to display.
...
As you can see, it is nothing more than a series of characters and string manipulation to achieve the formatting we want.
...
[ANSITerminal](https://github.com/pakLebah/ANSITerminal) is distributed as a Swift Package, so we can add it to our project by adding the following to our `Package.swift` file:
...
Before we start writing some code, let's take a look at what we'll be building in this article:
...
The picker, which is highly inspired by [Nate Moore's]() [clack javascript library](), presents any number of options and allows the user to select one using the arrow keys and the enter key.
...
Let's start by importing the [ANSITerminal]() library in the executable's entry point and creating a `picker` function where the logic will live:
...
> Note that in the following sections, unless I specifically say otherwise, all functions I will be calling in the `picker` method are either part of the `Foundation` or [ANSITerminal]() libraries.

### Code

The first thing we need to do is to show the picker's title:
...
1. First, we turn the cursor off to avoid having it displayed in the terminal as there will be no need for it.
2. Move one line down from the current position. This is needed because we want to start writing the title in a new line.
3. Write the icon for the picker. In this case, we are using the same Unicode character as [clack]() uses for its prompts.
4. Move the cursor one character to the right to add a white space and write the title.

### Output

![](https://res.cloudinary.com/dflewmkx4/image/upload/ac_none,f_auto,q_auto:eco/v1678222144/blog/posts/how-to-make-an-interactive-picker-for-a-swift-command-line-tool/title-prompt.png)
...
This state will hold the user's selected option and the range of lines where the options will be displayed.

Let's see how we can set up the initial state for the picker:
...
4. Write the icon for the option. If the option is active, use a green circle, otherwise, use a gray circle, similarly to what [clack]() does.
...
![](https://res.cloudinary.com/dflewmkx4/image/upload/ac_none,f_auto,q_auto:eco/v1678222426/blog/posts/how-to-make-an-interactive-picker-for-a-swift-command-line-tool/list-all-options.png)
