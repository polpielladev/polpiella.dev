...
The picker, which is highly inspired by [Nate Moore's](https://twitter.com/n_moore) [clack javascript library](https://github.com/natemoo-re/clack), presents any number of options and allows the user to select one using the arrow keys and the enter key.
...
> Note that in the following sections, unless I specifically say otherwise, all functions I will be calling in the `picker` method are either part of the `Foundation` or [ANSITerminal](https://github.com/pakLebah/ANSITerminal) libraries.
