...
This article goes through how you can create a custom view like the one shown in the video below built on top of the standard `TextField` component to unlock this behaviour:
...
The view in the preceding video behaves in the following way:
...
If you rely on this UI pattern as the sole means of editing a text item in your app (for example renaming items in a list), you can **make it easier for users to discover this feature** by adding a **context menu** action to the view.

The menu, which can become visible by right-clicking on the view, has a single button called 'Rename' which, when clicked on, focuses the `TextField` and makes it editable.
