...
The [2.211.0 release of Fastlane](https://github.com/fastlane/fastlane/releases/tag/2.211.0) comes with a new action to manage Xcode on CI.

As well as introducing a new lane, this release deprecates the existing [xcode-install](http://docs.fastlane.tools/actions/xcode_install) and [xcversion](https://docs.fastlane.tools/actions/xcversion) actions.
...
Both the existing [xcode_select](http://docs.fastlane.tools/actions/xcode_select) and the new [xcodes](https://docs.fastlane.tools/actions/xcodes) actions are able to achieve this by setting the `DEVELOPER_DIR` environment variable to the path of the selected Xcode application's contents.
...
The [xcode_select](http://docs.fastlane.tools/actions/xcode_select) action is the simplest and less intrusive way of selecting an Xcode version to use. The action requires the full path to an Xcode application (e.g. `/Applications/Xcode_14.1.app`) and it uses this path to set the `DEVELOPER_DIR` environment variable.
...
A big downside of this approach is that, since [xcode_select](http://docs.fastlane.tools/actions/xcode_select) requires a full path to the Xcode application, you need to have a strict naming convention for all Xcode installations across all your CI runners.

Even if you use self-hosted runners, if you want to run any of your lanes locally, you need to ensure that the path to your local Xcode installation matches that of the CI runner.
...
The downside to this new lane is that it does not work out-of-the-box as it relies on the system having [xcodes](https://github.com/RobotsAndPencils/xcodes) installed.
...
To make the [xcodes action](https://docs.fastlane.tools/actions/xcodes) install a version of Xcode if can't be found in the system, simply set the `select_for_current_build_only` parameter to `false` (or don't pass it at all as it defaults to `false`).
...
If you'd like to get some extra validation in place and make sure that the selected version of Xcode is the one you expect and valid, there are some extra actions you can use.
...
It is also worth mentioning that, similarly to the way the [xcodes](https://docs.fastlane.tools/actions/xcodes) action works, the [ensure_xcode_version action](https://docs.fastlane.tools/actions/ensure_xcode_version) also looks for a `.xcode-version` file in the root of the repository.

[ensure_xcode_version](https://docs.fastlane.tools/actions/ensure_xcode_version) looks for a version string in the `.xcode-version` file only if the `version` parameter isn't present in the action call.
