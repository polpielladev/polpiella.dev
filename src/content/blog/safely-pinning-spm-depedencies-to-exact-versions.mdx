...
excerpt: 'Specifying an exact version rule to dependencies in your Package.swift file is sometimes not enough to ensure that you are always building the same code from your third-party dependencies. In this article, I will show you how code might change across different builds even when depending on an exact version of a Swift Package.' 
pubDate: 2023-02-15
---

This week at work we have been looking into migrating 3rd-party CocoaPods dependencies to Swift Package Manager. We wanted to do this in a way where we maintained the same level of reproducibility and security that we currently had with CocoaPods or, if we couldn't, we reached an acceptable compromise. 
...
It is important to note that Apple are currently working on a Package registry which would improve the Swift Package Manager ecosystem dramatically and potentially mitigate issues as the one described in this article. 
