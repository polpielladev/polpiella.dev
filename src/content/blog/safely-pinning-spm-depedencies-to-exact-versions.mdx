...
As package resolution is a step of the build process, there is no easy and automated way of installing all Swift Package Manager dependencies into a *vendor* directory and committing it to source control like we do with [CocoaPods](https://cocoapods.org/). 
...
Even better, if you are using [fastlane](https://fastlane.tools/), you can set this flag by passing true to the `disable_pacakge_automatic_updates`  parameter in both the [gym](https://docs.fastlane.tools/actions/gym/) and [scan](https://docs.fastlane.tools/actions/scan/) actions 🎉:
