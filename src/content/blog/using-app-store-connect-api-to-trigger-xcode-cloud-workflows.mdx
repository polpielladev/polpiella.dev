...
Furthermore, since we want to select the correct product from its repository name, we need to specify that we want to include the `primaryRepositories` data associated with each product to response.
...
Now that the we have found the Xcode Cloud product we're looking for, we can get all available workflows via the same [ciProducts](https://developer.apple.com/documentation/appstoreconnectapi/list_all_xcode_cloud_products) endpoint but specifying an id and querying the `/workflows` subpath this time:
...
That's it! Running the `startWorflow` function will trigger a new workflow run! 🎉
