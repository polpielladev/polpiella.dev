...
We set out on a mission to replicate this behaviour in our SwiftUI navigation and tried a number of approaches with none of them worked in the way we wanted it to:
...
Once all this setup was done, we needed to create the root view controller for the application. As [the thread in Apple's Developer forums states](https://developer.apple.com/forums/thread/125155), we needed to make this a subclass of `UIHostingController`. This subclassed `UIHostingController` would then override the `supportedInterfaceOrientations` property and update it dynamically based on the values passed up by SwiftUI views using a preference.
...
Creating a preference is as easy as creating a struct which conforms to the `PreferenceKey` protocol, give it a default value and tell it how it should be updated by implementing the protocol's `reduce` method:
