...
Webhooks are messages that get sent when certain events occur on a website. These webhooks provide a way of building custom integrations based on the data they provide, hence allowing users to expand an app's capabilities through them.

Subscribing to webhook events is straightforward. You need to create a server which can handle `POST` requests and provide the server's URL to the website vending the webhook. Then, whenever the event occurs, the website sends a `POST` request to the provided URL.
...
<Video src="https://res.cloudinary.com/dflewmkx4/video/upload/ac_none,f_auto,q_auto:eco/v1678828603/blog/posts/github-webhooks-and-xcode-cloud/demo.mp4" />
...
![The cloud section in the report navigatior menu of Xcode](https://res.cloudinary.com/dflewmkx4/image/upload/ac_none,f_auto,q_auto:eco/v1678797532/blog/posts/github-webhooks-and-xcode-cloud/manage-workflows.png)

In the workflow list screen, click the `+` button.

![The workflow list screen with the plus button highlighted](https://res.cloudinary.com/dflewmkx4/image/upload/ac_none,f_auto,q_auto:eco/v1678797532/blog/posts/github-webhooks-and-xcode-cloud/create.png)

Change the default name of the workflow to 'TestFlight' and turn it off so that it does not get run every time a branch changes. This is the way to create a _manual_ workflow in Xcode Cloud.

![The default workflow configuration page with the name set to TestFlight and the toggle at the top switched off](https://res.cloudinary.com/dflewmkx4/image/upload/ac_none,f_auto,q_auto:eco/v1678797531/blog/posts/github-webhooks-and-xcode-cloud/manual-workflow.png)
...
![The workflow configuration page showing a start condition of any branch](https://res.cloudinary.com/dflewmkx4/image/upload/ac_none,f_auto,q_auto:eco/v1678797532/blog/posts/github-webhooks-and-xcode-cloud/archive-any-branch.png)

Add an archive action and select the `TestFlight (Internal Testing Only)` option.

![The workflow configuration page showing an action which archives the app for internal testing](https://res.cloudinary.com/dflewmkx4/image/upload/ac_none,f_auto,q_auto:eco/v1678798727/blog/posts/github-webhooks-and-xcode-cloud/archive-internal-testing.png)
...
![A post-action step to deliver the app to TestFlight and all internal testers](https://res.cloudinary.com/dflewmkx4/image/upload/ac_none,f_auto,q_auto:eco/v1678797531/blog/posts/github-webhooks-and-xcode-cloud/internal-testing.png)
...
Thanks to the great work from the [swif-server group](https://github.com/swift-server), you can create a lambda using Swift.
...
The lambda also needs to listen to `POST` requests sent by GitHub and, for this reason, it needs to have an Amazon API Gateway as its trigger.
...
From all the information in the payload, the webhook just needs the following information:
...
> The preceding payload is a simplified version with just the data that the function needs from GitHub's event data. You can use this example JSON to test the lambda locally.
...
The `APIProvider` gives you a way to interact with App Store Connect's API. It requires an `APIConfiguration` with your App Store Connect's credentials to make authenticated requests.
...
The lambda now has all the tools and information it needs to interact with the App Store Connect API and trigger a new `TestFlight` workflow run.
...
1. Making a network request to the [App Store Connect API's ciProducts endpoint](https://developer.apple.com/documentation/appstoreconnectapi/list_all_xcode_cloud_products) to retrieve all the products of type `app`.
2. Finding the product that matches the pull request's repository name by comparing the `repositoryName` property from the `scmRepository` included item with the pull request's repository name. If there are any matches, get the repository entity's `id`.
3. Finding the product that matches the repository entity's `id` by comparing the `repositoryId` property from the `product`'s `relationships` with the repository entity's `id`. If the repository has no matching product, return a `404` response.
...
2. Finding the `TestFlight` workflow by getting the first item in the workflows' array whose name matches `TestFlight`. If there are no matches, return a response with `404` status code.
...
After deploying the lambda you have just built, the last thing to do is to set up the webhook on the GitHub repository.

To do so, go to the repository's settings, navigate to the 'Webhooks' section and click the 'Add Webhook' button.

![A screenshot of the webhooks section in a GitHub repository](https://res.cloudinary.com/dflewmkx4/image/upload/ac_none,f_auto,q_auto:eco/v1678832075/blog/posts/github-webhooks-and-xcode-cloud/add-webhook.png)
...
![The new webook configuration page](https://res.cloudinary.com/dflewmkx4/image/upload/ac_none,f_auto,q_auto:eco/v1678832075/blog/posts/github-webhooks-and-xcode-cloud/create-webhook.png)

Check the 'Active' box to ensure the webhook is on and click the 'Add webhook' button to save the configuration.

![A page showing the active checkbox in the new webhook page](https://res.cloudinary.com/dflewmkx4/image/upload/ac_none,f_auto,q_auto:eco/v1678832075/blog/posts/github-webhooks-and-xcode-cloud/active.png)
