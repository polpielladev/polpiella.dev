...
To create a new workflow, go to the `Report Navigator` menu, select the `Cloud` menu, right click the target you want to create the new workflow for and select `Manage Workflows...`.
...
In the workflow list screen, click the `+` button to create a new workflow.
...
Change the name of the workflow to 'TestFlight' and turn off the workflow so that the workflow just runs manually.
...
The TestFlight workflow can now upload the app to TestFlight and distribute it to an internal test group. The workflow is also off so that it just runs when you manually trigger it rather than on any branch change.
