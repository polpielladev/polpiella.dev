...
While the GitHub [issue_comment]() webhook sends some of the pull request's information, it doesn't send enough to determine the pull request's head branch.
...
This property is a URL pointing to the [pulls]() GitHub endpoint, which requires authentication for private repositories. The lambda can then access the `head.ref` property from GitHub's response to get the pull request's head's branch name.
...
## Setting up the webhook for a repository

After deploying the lambda you have just built, the last thing to do is to set up the webhook on a GitHub's repository.

To do so, go to the repository's settings, navigate to the 'Webhooks' section and click the 'Add Webhook' button.

![]()

Enter the URL for the deployed server in the 'Payload URL' field, select 'application/json' as the 'Content type', and choose the 'Let me select individual events option'. From the list of available events, select 'Issue Comments'.

![]()

Check the 'Active' box to ensure the webhook sends events and click the 'Add webhook' button to save the configuration.
