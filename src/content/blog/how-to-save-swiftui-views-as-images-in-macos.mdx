...
As it stands, SwiftUI does not have a native way to allow the user to save a file to disk from a macOS app using a panel.

However, you can still use `AppKit` to create an `NSPanel` and present it from a SwiftUI view.
...
The function above creates an `NSPanel`, sets the allowed content types, shows the panel to the user using the `runModal` method and if the response is valid it then returns the URL where the new file will be created.
