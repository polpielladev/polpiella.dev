...
I did a bit of research and, thanks to [this amazing Stack Overflow answer](https://stackoverflow.com/a/58297743), I was pointed to an initializer of `Binding` I was not aware of: `init?(_ base: Binding<Value?>)`. In a nutshell, what this initializer does is **unwrap the optional value** the `Binding` is holding and instead provide an optional `Binding` with a **non-optional value**.
