...
To do this, we created a script that would retrieve the access token from the environemnt (we pass it through a secret in the repository) and store it in the cache using the `git credential-cache` command:
...
The beauty about this command, which we found in [this gist by Rober Citek](https://gist.github.com/rwcitek/da862e9e27cc28d3e96e62a2ca4b2b64), is that it will call `git credential-cache store` without having to create a file with the credentials in it first 🎉.
...
First, we created a new lane to remove the access token from the in-memory cache. This is as simple as stopping the credential helper:
...
Then, we wrapped our build commands in a `begin/ensure` block to make sure that the credentials are cached and removed at the right time:
