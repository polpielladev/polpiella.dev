...
> The formula has a description and a URL to the release archive with the corresponding SHA256 checksum. It also has an `install` method which tells Homebrew to install an executable called 'chatty' (found at the root of the tarball) to the user's brew binaries directory.
