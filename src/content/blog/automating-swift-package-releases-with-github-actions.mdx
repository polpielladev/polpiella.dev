...
To distribute the application, I first needed to build it for the platforms I wanted to support (macOS arm64 and x86_64). To do this, I needed to tell the workflow to run on the latest macOS version, check out the repository and build the executable product with two architecture slices using `swift build`:
...
I then needed to create a GitHub release with the tag name and upload a compressed archive of the previous step's artefact to the GitHub release:
...
> The naming of the tarball archive is important as it is used to determine the URL of the Homebrew formula's executable.
...
As I mentioned earlier in the article, I wanted to make [chatty](https://github.com/polpielladev/chatty-cli) available on Homebrew as it is the most common way of installing command line tools on macOS.

To distribute my tool on [Homebrew](https://brew.sh), I had to publish a formula. I reused an existing [Homebrew tap that I had created for another project](https://github.com/polpielladev/homebrew-tap) and added a brand new formula for an existing release of [chatty](https://github.com/polpielladev/chatty-cli):
...
I then used it in a step directly after creating the GitHub release in the workflow:
...
I had to give the homebrew action the name of the formula (`formula-name`), the name of my personal Homebrew tap (`polpielladev/homebrew-tap`) as it otherwise defaults to `Homebrew/homebrew-core` and the name of the branch to commit to in the tap (`main`).
