...
Now that we have all the necessary data, we can use it to generate the image's UI and render it as a `png` image. The most common way of doing this is to use a headless browser like [Puppeteer]() or [Playwright]() to render HTML and CSS and then take a screenshot of the result.
...
After generating the SVG using [satori-html], we convert it to a `png` using [resvg](), which is a Rust library that converts SVGs to `png` images, and we return it as the response to the request:
