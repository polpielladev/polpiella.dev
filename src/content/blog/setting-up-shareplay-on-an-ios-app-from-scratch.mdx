---
title: 'Setting up SharePlay on an iOS app from scratch'
excerpt: ''
pubDate: 2024-02-07
---

I have recently been working on a feature for [NowPlaying]() that leverages the SarePlay APIs to allow users to join listening sessions with their friends.

It is the first time I have worked with SharePlay, and I found the documentation and process to set it up a bit confusing, so I decided to write this post to help others who might be in the same situation.
...
In the "Signing & Capabilities" tab, click the "+" button, search for "SharePlay" and add the capability.

![]()

## Creating a GroupActivity

SharePlay experiences are built around the concept of `GroupActivity`s, which are types that represent an app's shareable experiences. Creating one is as simple as defining a type (or using an existing one) and making it conform to the `GroupActivity` protocol.
...
- If your app is already in a FaceTime call, you can start the SharePlay session directly.
- If your app is not in a FaceTime call, you can let the user start a session by presenting a ``.
...
5. If the user is not in a FaceTime call, return `.needsDialog` to prompt the user to share the activity.
...
If the user is eligible for group sessions, the SharePlay session can begin immediately. If not, you will need to handle the outcome and share the activity with their friends using a [GroupActivitySharingController]():
...
To get an instance of the current SharePlay session, you need to use the `sessions()` static property on your `GroupActivity` type. As this property returns an AsyncStream, you can use it to subscribe to listen for new sessions and keep track of the currently active one:
...
As you can see in the snippet above, as soon as a new session is available, you can let your user join it by calling the `join` method and then start listening for messages using a newly created instance of the `GroupSessionMessenger`.

As was the case with the sessions, `messages` is an AsyncStream, so you can use structured concurrency to listen for any new messages and handle them accordingly.
...
Note that you can send any `Codable` type as a message, but be careful to not send large payloads as otherwise, SharePlay will throw an error.
...
This particular use case is something that I struggled with during my implementation. There are two ways to end a SharePlay session, and you will want to handle both of them:
...
Let's first handle the first case and allow users to end the session from the app:
...
Now, as the session will be invalidated, you will need to handle the change of state and clean up any resources for all the participants and not just the one that ended the session. You can do so by listening to the `$state` property on the `GroupSession`:
