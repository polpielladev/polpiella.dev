...
I have been using a **Swift AWS lambda** to automatically post a tweet whenever a new issue of the [iOS CI Newsletter](https://www.polpiella.dev/newsletter) is sent to all subscribers.

I use the campaign sent webhook event from [Sendy](https://sendy.co) to trigger new lambda runs. The lambda then uses the event's payload to get the issue number, the number of subscribers the email was sent to and the featured authors' Twitter profiles to compose the tweet's body and uses Twitter's API to post it.
...
Before I continue, I wanted to quickly note that my initial plan was to use the [swift package archive]() command on CI/CD to build and package the lambda's code. Unfortunately, I ran into [an issue where the plugin execution was failing on CI/CD](https://github.com/polpielladev/sendy-webhooks/actions/runs/5113000554/jobs/9191696719) and I had to resort to using a bash script instead.
