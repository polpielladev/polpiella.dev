...
When specifying that a plugin requires the `allowNetworkConnections` permission we must also specify a **scope**, which determines the requests that can or can not be made from the plugin's code.
...
> If you want to learn more about how to download and install development snapshopts of Swift, [check out my article on the topic](https://www.polpiella.dev/managing-multiple-swift-versions-locally).
...
Despite that it is still early days in the development of this new permission and it won't be available until Swift 5.9, I am very excited about the amount of possibilities it brings to Swift package plugins and the ability to write more complex plugins without having to disable the sandboxed environment they run on.

And having to disable the sandbox to make network requests is [the issue that sparked the interest](https://github.com/apple/swift-package-manager/issues/5489) for this feature in Swift Package Manager. The [swift-aws-lambda-runtime](https://github.com/swift-server/swift-aws-lambda-runtime/pull/254) project has a plugin which needs to interact with Docker to package an executable target into a format suitable for upload to AWS. This plugin can only be executed by passing the `--disable-sandbox` flag to the `swift package` command, which is not ideal as it can have a number of undesired side effects.
